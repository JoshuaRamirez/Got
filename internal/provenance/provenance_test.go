...
func TestCausesConnectedChain(t *testing.T) {
	...
	for _, p := range pairs {
		ok, err := e.Causes(ctx, g, vid(p[0]), vid(p[1]))
		if err != nil {
			t.Fatal(err)
		}
		if !ok {
...
func TestCausesReflexive(t *testing.T) {
	...
	g := buildChain(t)
	e := newEngine()
	ok, _ := e.Causes(ctx, g, vid("exec"), vid("exec"))
	if !ok {
...
func TestNonCausalEdgeExcluded(t *testing.T) {
	...
	e := newEngine()
	ok, _ := e.Causes(ctx, g, agent, artifact)
	if ok {
...
func TestTraceSet(t *testing.T) {
	...
	traces, err := e.TraceSet(ctx, g, vid("model"), vid("artifact"))
	if err != nil {
		t.Fatal(err)
	}
	if len(traces) == 0 {
...
func TestCloseIsolatedVertex(t *testing.T) {
	...
	e := newEngine()
	closed, _ := e.Close(ctx, g, []identity.VertexID{lonely})
