...
| [UC-U18](user/UC-U18-three-way-merge.md) | Three-way merge against a common ancestor | Verified | `internal/composition/threeway.go` (`DefaultEngine.MergeThreeWay`, `ThreeWayMerger`); facade passthrough `internal/repo/service.go` (`DefaultService.MergeThreeWay`) | `internal/composition/threeway_test.go`, `internal/repo/repo_test.go` | 2026-06-11 | Additive concrete method (no Engine interface change). Ancestor-relative reconciliation: only-left/only-right/agreed change, add, honored deletion, both-delete success paths; modify/modify, add/add, modify/delete, schema, and Unsat-policy conflict paths; plain-frontier presence-only degradation; ctx cancellation. Content via projection.Edited. Facade `repo.Service.MergeThreeWay` exposes it end-to-end via a `composition.ThreeWayMerger` type-assert; two-way-only engines yield `repo.ErrThreeWayUnsupported` (both paths tested). |
| [UC-U19](user/UC-U19-operate-from-cli.md) | Operate the repository from the command line | Verified | `cmd/got` (`run.go`, `store.go`, `helpers.go`) | `cmd/got/run_test.go` | 2026-06-11 | CLI shell over the library with JSON persistence under $GOT_DIR. init/add-vertex/add-edge/bind/resolve/list/trace/cone plus revise/merge/materialize/release commands drive repo.Ingest, repo.Revise, repo.Branch, repo.Merge, repo.MergeThreeWay, repo.Materialize, repo.Release, namespace.ResolveRef, and provenance.Engine. Tests cover happy paths plus unknown-command, before-init, unknown-type, duplicate, inadmissible-edge (state unchanged), missing-endpoint, bind-unknown, resolve-unbound, unconnected-trace, revise non-Artifact/unknown anchor, merge missing-flags/unknown-vertex, three-way addition, unsupported materialize target, release happy/unknown-vertex, and cross-invocation persistence. New delivery channel for UC-U01/U02/U03/U04/U06/U07/U09/U10/U11/U18/S08 — no new engine behavior. |
